...
class Solution:
    def findWords(self, board: List[List[str]], words: List[str]) -> List[str]:
        ...
        def findWord(word, node: Node, position):
            if node.final:
            ...
            for neighbor in getNeighbors(position):
                ...
                if neighborChar == '*' or neighborChar not in node.children:
                    continue
                word.append(neighborChar)
                board[neighbor[1]][neighbor[0]] = '*'
                findWord(word, node.children[neighborChar], neighbor)
                board[neighbor[1]][neighbor[0]] = neighborChar
                word.pop()
